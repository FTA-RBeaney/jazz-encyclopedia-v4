...
export default defineEventHandler(async (event: H3Event) => {
  try {
    ...
    // 1. Fetch popular videos from Supabase
    ...
    if (error) {
      console.error("Error fetching popular videos from Supabase:", error);
      throw createError({
        statusCode: 500,
        statusMessage: "Failed to fetch popular videos",
      });
    }
    ...
    throw createError({
      statusCode: 500,
      statusMessage: "Failed to process popular videos",
