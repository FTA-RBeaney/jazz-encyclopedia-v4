export default defineEventHandler(async (event) => {
  ...
  const release_id = event.context.params?.id;

  if (!release_id) return { error: "Release ID is required" };

  const url = `https://api.discogs.com/releases/${release_id}?token=${apiKey}`;
